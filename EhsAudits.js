...
function renderOpenAuditsTable() {
    ...
    // Ordenar por Folio de mayor a menor
    openAudits.sort((a, b) => {
...
// Función para habilitar la edición de campos en la fila seleccionada
function enableEditRow(buttonElement, rowId) {
    ...
    // Columna Status (Lista: In Process / Completed)
    ...
    // Cambiar icono de Editar por icono de Guardar
...
// Función para guardar los cambios y reemplazar la información en la tabla de Supabase
async function saveEditedRow(buttonElement, rowId) {
    ...
    // Obtener valores nuevos de los inputs de texto
    const fieldsToEdit = ["Folio", "Shift", "Area", "EHSName", "Gerente", "Fecha"];
    fieldsToEdit.forEach(fieldName => {
        const td = tr.querySelector(`td[data-field="${fieldName}"]`);
        if (td) {
    ...
    // Realizar actualización (Update) en Supabase
    // Nota: Si tu llave primaria en la base de datos se llama 'id' o 'Id', ajustamos el filtro de búsqueda.
    const { error } = await supabaseClient
        .from("Auditorias_Gestion_EHS")
        .update(updatedData)
        .or(`Id.eq.${rowId},id.eq.${rowId}`);
    ...
    await loadMaintenance(); // Recargar datos y refrescar la interfaz
}
...
function setProtectedElementsState(isLoggedIn, userName = "") {
    isUserLoggedIn = isLoggedIn; // Actualiza el estatus global
    ...
    // Refrescar la tabla para habilitar/deshabilitar los iconos de editar en la columna Actions
    renderOpenAuditsTable();
}
...
function handleDataClick() {
    if (sessionActiveUser) {
        alert('¡Sesión activa! Abriendo Data...');
    } else {
