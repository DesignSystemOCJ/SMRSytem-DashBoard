...
const menuConfig = [
  { home: true, href: "#", tooltip: "Home", icon: "fa-solid fa-house", label: "Home" }, // <-- Home añadido correctamente para la computadora
  {
    type: "dropdown",
  ...
  if (sidebarMenu) {
    sidebarMenu.innerHTML = menuConfig.map(item => {
      ...
      if (item.type === "dropdown") {
        ...
        const targetAttr = item.home ? '' : 'target="content-frame"';
        
        return `
          <li class="sidebar-item">
            <a href="${hrefAttr}" ${targetAttr} class="sidebar-link" ${idAttr} data-tooltip="${item.tooltip}" ${homeAttr}>
              <span class="nav-icon"><i class="${item.icon}" aria-hidden="true"></i></span>
              <span class="nav-label">${item.label}</span>
            </a>
          </li>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (DOM.sidebar) {
    ...
    DOM.sidebar.addEventListener("click", (event) => {
      ...
      // Detección del clic en el botón Home de la sidebar de computadora
...
function updateMenuLoginState() {
  ...
  if (currentSessionUser) {
    ...
    if (userAvatarImg) {
      userAvatarImg.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(currentSessionUser)}&background=10b981&color=fff`;
      userAvatarImg.style.display = "block";
    }

    if (userAvatarBox) {
      userAvatarBox.classList.remove("logged-out");
      userAvatarBox.classList.add("logged-in");
    }
    ...
    if (userAvatarBox) {
      userAvatarBox.classList.remove("logged-in");
      userAvatarBox.classList.add("logged-out");
    }
