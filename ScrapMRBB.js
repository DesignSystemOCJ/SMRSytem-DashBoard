...
// Funciones de renderizado dinámico añadidas
function renderMonthDropdown() {
    const dropdown = document.getElementById("monthDropdown");
    if (!dropdown) return;

    let content = '<div class="dropdown-title">Select Scrap Period</div>';
    months.forEach(month => {
        content += `<div class="custom-option" onclick="selectMonth('${month}', event)">${month}</div>`;
    });
    dropdown.innerHTML = content;
}

function renderModalMonthSelect() {
    const select = document.getElementById("modalMonthSelect");
    if (!select) return;
...
async function fetchLatestProductionDate() {
    ...
    try {
        ...
        if (data && data.length > 0 && data[0].Date) {
            const rawDate = data[0].Date.split("T")[0];
