...
window.addEventListener("click", function(event) {
    ...
    if (event.target === updateProdModal) closeUpdateProdModal();
    if (event.target === updateDataModal) closeUpdateDataModal();
...
function setupStorageListener() {
    window.addEventListener("storage", function(event) {
        if (event.key === "smrc_logged_user") {
            if (event.newValue) {
                setProtectedElementsState(true, event.newValue);
            } else {
                setProtectedElementsState(false);
            }
        }
...
function setProtectedElementsState(isLoggedIn, userName = "") {
    ...
    const updateProdOption = document.getElementById("updateProdMenuOption");
    ...
    if (isLoggedIn) {
        ...
        if (updateProdOption) {
            updateProdOption.style.opacity = "1";
            updateProdOption.style.pointerEvents = "auto";
            updateProdOption.style.cursor = "pointer";
        }
        if (statusText) {
            statusText.textContent = userName;
            statusText.style.color = "#000000"; // Color negro al iniciar sesión
        }
        ...
        if (updateProdOption) {
            updateProdOption.style.opacity = "0.5";
            updateProdOption.style.pointerEvents = "none";
            updateProdOption.style.cursor = "not-allowed";
        }
        if (statusText) {
            statusText.textContent = "NO LOG IN";
            statusText.style.color = "#000000"; // Color negro en estado sin sesión
        }
        if (avatarText) {
            avatarText.textContent = "RC";
        }
    }
}
...
async function fetchLatestProductionDate() {
    const badgeElem = document.getElementById("lastProdDateBadge");
    if (!badgeElem) return;
    ...
    try {
        ...
        // Consultamos ordenando de forma descendente por la columna Date para obtener la más reciente
        const { data, error } = await client
            .from("Production")
            .select("Date")
        ...
        if (data && data.length > 0 && data[0].Date) {
            const rawDate = data[0].Date.split("T")[0]; // Formato YYYY-MM-DD
            badgeElem.textContent = rawDate;
        } else {
            badgeElem.textContent = "No records";
        }
    } catch (err) {
        console.error("Error fetching latest production date:", err);
        badgeElem.textContent = "Error";
    }
}

function handleUpdateProdClick() {
    if (sessionActiveUser) {
        const modal = document.getElementById("updateProdModal");
        if (modal) {
            modal.style.display = "flex";
            fetchLatestProductionDate(); // Cargamos el último dato al abrir el modal
        }
    } else {
        alert('You must log in to access this option.');
    }
...
async function handleUpdateProdSubmit(event) {
    event.preventDefault();

    const dateVal = document.getElementById("prodDate").value;
    const monthVal = document.getElementById("prodMonth").value;
    const runningVal = document.getElementById("prodRunning").value;
    const scrapVal = document.getElementById("prodScrap").value;
    const avegVal = document.getElementById("prodAveg").value;

    try {
        const client = window.supabaseClient || supabaseClient;
        if (!client) throw new Error("Supabase client not initialized.");

        // Obtener el conteo total de registros para calcular el siguiente número (No.)
        ...
        let nextNo = (count !== null ? count : 0) + 1;

        // Construir el payload con el número calculado
        const payload = {
            "No.": nextNo,
            "Date": dateVal,
            "Month": monthVal,
            "Running": parseFloat(runningVal),
            "Scrap": parseFloat(scrapVal),
            "%Aveg": parseFloat(avegVal)
        };
        ...
        document.getElementById("updateProdForm").reset();
        
        // Actualizamos la fecha del header y los datos generales si corresponde
        fetchLatestProductionDate();
        
        if (currentSelectedMonth === monthVal) {
            await updateTopAreaAverage(currentSelectedMonth);
            updateDashboard();
        }
    } catch (err) {
        console.error("Error inserting into Production table:", err);
        alert("Error saving record: " + err.message);
    }
}

function handleProdDateChange(dateValue) {
    const monthInput = document.getElementById("prodMonth");
    ...
    // Dividir la fecha "YYYY-MM-DD" para evitar desfases de zona horaria
    ...
    if (parts.length === 3) {
        ...
        // Obtener el nombre del mes en inglés (coincidente con tus selectores)
