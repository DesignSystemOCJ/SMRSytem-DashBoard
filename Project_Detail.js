...
async function cargarProyecto() {
    ...
    document.getElementById("RemarksComments").value = data.RemarksComments ?? "";

    document.getElementById("Evidencebefore").src = data.EvidenceBefore
        ? data.EvidenceBefore
        : "https://via.placeholder.com/400x250?text=No+Image";

    document.getElementById("EvidenceAfter").src = data.EvidenceAfter
        ? data.EvidenceAfter
        : "https://via.placeholder.com/400x250?text=No+Image";
}
...
async function subirImagen(file, nombre) {
    ...
    const url = supabaseUrl + "/storage/v1/object/public/evidenceproject/" + nombre;
    return url;
}
...
async function guardarProyecto() {
    ...
    try {
        ...
        const beforeFile = document.getElementById("EvidenceBeforeFile").files[0];
        const afterFile = document.getElementById("EvidenceAfterFile").files[0];

        if (beforeFile) {
            evidenceBeforeURL = await subirImagen(
                beforeFile,
        ...
        if (afterFile) {
            evidenceAfterURL = await subirImagen(
                afterFile,
        ...
        if (evidenceBeforeURL) {
            cambios.EvidenceBefore = evidenceBeforeURL;
        }

        if (evidenceAfterURL) {
            cambios.EvidenceAfter = evidenceAfterURL;
        }
        ...
        if (!data || data.length === 0) {
            alert("❌ No se encontró el proyecto para actualizar");
            return;
        }
