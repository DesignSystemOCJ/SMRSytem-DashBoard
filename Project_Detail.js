/* =========================================================
   PROJECT DETAIL
   VERSION CORREGIDA Y ACTUALIZADA
========================================================= */
...
/* =========================================================
   LLENAR SELECTORES DINÁMICAMENTE (HTML to JS)
========================================================= */

function inicializarSelectores() {
    // 1. Selector de Estado (Status)
    const selectStatus = document.getElementById("Status");
    if (selectStatus) {
        const estados = [
            "New", "Under Review", "In Process", 
            "On Hold", "Completed", "Closed", "Canceled"
        ];
    ...
    if (selectLead) {
        const leads = [
            "Demetrio Moreno", 
            "Armando Osornio", 
            "Demetrio Moreno - Armando Osornio", 
            "Armando Osornio - Demetrio Moreno"
        ];

        selectLead.innerHTML = '<option value="">Select lead...</option>';
        leads.forEach(lead => {
            const opt = document.createElement("option");
            opt.value = lead;
    ...
    // 3. Selector de Manager
    const selectManager = document.getElementById("Manager");
    if (selectManager) {
        const managers = [
            "Ernesto Guerrero", 
            "Jorge Garcia", 
        ...
        selectManager.innerHTML = '<option value="">Select manager...</option>';
...
/* =========================================================
   VISOR FLOTANTE DE IMÁGENES (ZOOM POR CLIC)
========================================================= */

function inicializarVisorEvidencias() {
    const idsImagenes = ["Evidencebefore", "EvidenceAfter"];

    idsImagenes.forEach(id => {
        const imgElement = document.getElementById(id);
        if (!imgElement) return;

        const frame = imgElement.closest(".image-frame");
        if (frame) {
            frame.addEventListener("click", () => {
                const src = imgElement.src;
                if (src && src.trim() !== "" && !src.endsWith(window.location.href)) {
                    abrirVisorImagen(src);
                } else {
                    alert("No hay imagen disponible para ampliar.");
                }
            });
        }
    });
...
function abrirVisorImagen(src) {
    ...
    const modalImg = document.getElementById("modalImageEnlarged");
    if (modal && modalImg) {
        modalImg.src = src;
...
function cancelarEdicion() {
    cargarProyecto(); // Restaura datos originales de la BD
    modoLectura();    // Regresa al estado inicial de botones
}
...
document.addEventListener(
    ...
    function() {
        ...
        // Inicializamos los selectores por medio de JavaScript
        inicializarSelectores();

        // Inicializamos el visor flotante de evidencias
        inicializarVisorEvidencias();
