...
// Columnas actualizadas según tu tabla de Supabase
...
async function loadModalRecords() {
    ...
    // Filtrado robusto por año basado en la columna Date
    ...
    if (modalRawData.length === 0) {
        ...
        tbody.innerHTML = `<tr><td colspan="${desiredModalColumns.length}" style="text-align:center; padding:40px;">No records found for <strong>${selectedYear}</strong></td></tr>`;
...
function filterModalTable() {
    ...
    // Filtra utilizando 'Depart' (o de respaldo 'Maq')
    ...
    if (filtered.length === 0) {
        tbody.innerHTML = `<tr><td colspan="${desiredModalColumns.length}" style="text-align:center; padding:40px;">No records match the selected filter.</td></tr>`;
        return;
    }
