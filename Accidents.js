...
// Función para poblar dinámicamente los selectores de año en la interfaz
function populateYearDropdowns() {
    // 1. Poblar el menú desplegable superior
    ...
    // 2. Poblar el select del modal de registros
...
window.addEventListener("load", function() {
    populateYearDropdowns(); // Genera dinámicamente los años en la interfaz
    setCurrentYear();
...
document.addEventListener("keydown", (e) => {
    ...
    if (e.ctrlKey && (key === "u" || key === "c" || key === "x" || key === "s" || key === "a")) {
        e.preventDefault();
        return;
    }
});